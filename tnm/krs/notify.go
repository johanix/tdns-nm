...
func HandleKrsNotify(ctx context.Context, dnr *tdns.DnsNotifyRequest, krsDB *KrsDB, conf *tnm.KrsConf) error {
	...
	if len(msg.Question) > 0 {
		notifyZone = msg.Question[0].Name
		log.Printf("KRS: Received NOTIFY for zone: %s (Qtype: %s)", notifyZone, dns.TypeToString[msg.Question[0].Qtype])
	} else {
